    ...
    if TRAIN_MODELS:
        ...
        if EVALUATE_MODEL:
            ...
            if not os.path.exists(model_path):
                ...
                # <<< NEU: Definieren Sie ein Limit für die maximalen Schritte pro Episode >>>
                ...
                for episode in range(eval_episodes):
                    ...
                    # <<< NEU: Initialisieren Sie den Schrittzähler für die aktuelle Episode >>>
                    current_steps = 0

                    while not done:
                        # <<< NEU: Schrittzähler bei jeder Aktion erhöhen >>>
                        current_steps += 1
                        ...
                        # <<< NEU: Prüfen, ob das Schrittlimit erreicht wurde >>>
                        if current_steps >= max_steps_per_episode:
                            print(
                                f"  [WARNUNG] Episode {episode + 1} bei {current_steps} Schritten abgebrochen (Limit erreicht).")
                            done = True  # Beendet die Schleife künstlich

                    # Dieser Teil wird nun auch ausgeführt, wenn die Episode abgebrochen wurde
                    output_dir = "dqn_dynamic_layout"

                    # Speichern des Popularitätsverlaufs
                    ...
                    # Speichern des finalen Layouts (oder des Layouts beim Abbruch)
